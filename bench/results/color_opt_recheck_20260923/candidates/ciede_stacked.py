...
def _lab_f(t: torch.Tensor) -> torch.Tensor:
    ...
    return torch.where(
        t > _DELTA3, t.clamp_min(0.0).pow(1.0 / 3.0), t / _DELTA2_3 + _DELTA_OFF
    )
...
def _de00_planes(L1, a1, b1, L2, a2, b2, kL: float, kC: float, kH: float):
    ...
    dHp = 2.0 * torch.sqrt(C1p * C2p) * torch.sin(dhp * (math.pi / 360.0))
    ...
    Cp7 = Cb64**7
    Rc = 2.0 * torch.sqrt(Cp7 / (Cp7 + _P25_7))
    Ld = Lb64 - 50.0
...
def _rgb_pair_to_demap(x4, y4, inv_l: float, cr, cg, cb,
    ...
    lab = _convert_one(torch.cat((x4, y4), dim=0), inv_l, cr, cg, cb)
    n = x4.shape[0]
    lab1, lab2 = lab[:n], lab[n:]
    return _de00_planes(
